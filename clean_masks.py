...
"""""
    def preload_images(self):
        start_index = max(0, self.index - self.cache_size // 2)
        end_index = min(len(self.image_files), start_index + self.cache_size)

        for i in range(start_index, end_index):
            if i not in self.image_cache:
                image_path = os.path.join(self.images_folder, self.image_files[i])
                mask_path = os.path.join(self.masks_folder, self.mask_files[i])
                ...
                new_width = 640
                new_height = 480
                current_image = cv2.resize(overlay, (new_width, new_height), interpolation=cv2.INTER_AREA)
                current_image = cv2.cvtColor(current_image, cv2.COLOR_BGR2RGBA)
                photo = ImageTk.PhotoImage(image=Image.fromarray(current_image))
